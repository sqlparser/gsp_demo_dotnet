...
The existing filter contains `OR`, so the demo explicitly creates a parenthesis
node around that expression before joining the tenant predicate with an `AND`
node. This preserves the original boolean meaning and illustrates why AST
modification is safer and more maintainable than regular-expression or string
replacement.

Run it from the repository root:

```bash
dotnet run --project src/demos/modifySqlAst/demos.modifySqlAst.csproj -c Release
```
